...
from username_checker.common.settings.models.broker import TasksBrokerType
from username_checker.infrastructure.tkq.override import PullBasedJetStreamBrokerDI, PushBasedJetStreamBrokerDI


def create_tasks_broker(settings: Settings) -> AsyncBroker:
    """
    Creates a Tasks Broker based on the given Broker Settings.

    :param settings: (Settings): The settings object containing the configuration for the Tasks Broker.

    :return: (AsyncBroker): The created Tasks Broker.
    """
    nats_settings = settings.broker.nats
